...
// OpenURLInChromeProfile launches Chrome explicitly using the designated user profile directory.
func OpenURLInChromeProfile(urlStr, profileDir string) error {
	chromePath := FindChromeExecutable()
	args := []string{fmt.Sprintf("--profile-directory=%s", profileDir), urlStr}
